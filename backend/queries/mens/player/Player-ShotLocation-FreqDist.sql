-- Basketball Analytics Query: Player shot distribution frequency analysis
-- Shows shot distribution as percentages across the four court zones
-- Each player's four percentages sum to 100% of their total shooting plays
...
WITH shot_frequency AS (
    ...
    GROUP BY 
        ...
        primary_team
    HAVING 
        -- At least 15 total shot attempts
        (SUM(CASE WHEN shot_level = 1 AND outcome IN ('2pmi', '2pMa') THEN 1 ELSE 0 END) +
         SUM(CASE WHEN shot_level = 2 THEN 1 ELSE 0 END) +
         SUM(CASE WHEN shot_level = 3 THEN 1 ELSE 0 END) +
         SUM(CASE WHEN shot_level = 4 THEN 1 ELSE 0 END)) >= 1
)
SELECT 
    PLAYER,
    TEAM,
