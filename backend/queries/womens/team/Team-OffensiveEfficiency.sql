-- Basketball Analytics Query: Team performance by play type with percentile rankings
-- Analyzes shooting efficiency (2P%, 3P%, PPP) for teams with 30+ plays of each type
-- Calculates percentile ranks within play types (2P% rank requires 30+ 2PA, 3P% rank requires 20+ 3PA)
-- USE RITMensBasketball;
USE RITWomensBasketball;
WITH team_play_stats AS (
    SELECT 
        CASE 
        ...
        ROUND(	(SUM(CASE WHEN outcome = '2pMa' THEN 1 ELSE 0 END) / 
                 NULLIF((SUM(CASE WHEN outcome = '2pmi' THEN 1 ELSE 0 END) + 
                         SUM(CASE WHEN outcome = '2pMa' THEN 1 ELSE 0 END)), 0) * 100), 2 ) AS "2P%",
        -- 3FG Statistics
        ...
        ROUND(	(SUM(CASE WHEN outcome = '3pMa' THEN 1 ELSE 0 END) / 
                 NULLIF((SUM(CASE WHEN outcome = '3pmi' THEN 1 ELSE 0 END) + 
                         SUM(CASE WHEN outcome = '3pMa' THEN 1 ELSE 0 END)), 0) * 100), 2 ) AS "3P%",
        -- Play Count
        COUNT(*) as PLAY_COUNT
    FROM 
...
team_play_data AS (
    ...
    FROM team_play_stats
)
SELECT * FROM team_play_data
