/**
 * Women's Schedule Update Utilities
 * 
 * This file provides functions to update the women's basketball schedule.
 * Import these functions to programmatically update schedule data.
 * 
 * Usage:
 *   import { updateGameResult, updateGameDate, updateTeamLogo } from './upd_womens_schedule';
 *   
 *   // Update a game result
 *   updateGameResult(9, 'win', '68-55');
 *   
 *   // Update a game date
 *   updateGameDate(9, 'January 4, 2026', 'Sunday', '2:00 PM');
 *   
 *   // Update a team logo
 *   updateTeamLogo(9, '/imgs/logos/new-logo.png');
 */

import { womensSchedule, womensSummary } from './womensSchedule';

/**
 * Updates the result of a specific game
 * @param {number} gameId - The ID of the game to update
 * @param {string} resultType - 'win' or 'loss'
 * @param {string} score - The score string (e.g., '68-55')
 * @returns {Object} - The updated game object or null if not found
 */
export const updateGameResult = (gameId, resultType, score) => {
  const gameIndex = womensSchedule.findIndex(game => game.id === gameId);
  ...
  womensSchedule[gameIndex].result = {
    type: resultType,
    score: score
  };
  womensSchedule[gameIndex].completed = true;
  
  // Update summary records
  recalculateSummary();
  
  console.log(`Updated game ${gameId}: ${resultType} ${score}`);
  return womensSchedule[gameIndex];
};

/**
 * Updates the date and time of a specific game
 * @param {number} gameId - The ID of the game to update
 * @param {string} date - The new date (e.g., 'January 4, 2026')
 * @param {string} day - The day of the week (e.g., 'Sunday')
 * @param {string} time - The new time (e.g., '2:00 PM')
 * @returns {Object} - The updated game object or null if not found
 */
export const updateGameDate = (gameId, date, day, time) => {
  ...
  womensSchedule[gameIndex].date = date;
  womensSchedule[gameIndex].day = day;
  womensSchedule[gameIndex].time = time;
...
export const updateTeamLogo = (gameId, logoPath) => {
  ...
  womensSchedule[gameIndex].logo = logoPath;
...
export const updateGameLocation = (gameId, location) => {
  ...
  womensSchedule[gameIndex].location = location;
...
export const updateGameHomeAway = (gameId, homeAway) => {
  ...
  womensSchedule[gameIndex].homeAway = homeAway;
...
export const updateGameNotes = (gameId, notes) => {
  ...
  womensSchedule[gameIndex].notes = notes;
...
export const getGame = (gameId) => {
  return womensSchedule.find(game => game.id === gameId) || null;
};
...
export const getCompletedGames = () => {
  return womensSchedule.filter(game => game.completed);
};
...
export const getLeagueGames = () => {
  return womensSchedule.filter(game => game.conference === 'Liberty League');
};
...
const recalculateSummary = () => {
  const completedGames = womensSchedule.filter(game => game.completed);
  const leagueGames = completedGames.filter(game => game.conference === 'Liberty League');
  
  womensSummary.completedGames = completedGames.length;
  womensSummary.remainingGames = womensSchedule.length - womensSummary.completedGames;
  
  womensSummary.overallRecord = {
...
export const exportSchedule = () => {
  return JSON.stringify({
    schedule: womensSchedule,
    summary: womensSummary
  }, null, 2);
};

export { womensSchedule, womensSummary };
