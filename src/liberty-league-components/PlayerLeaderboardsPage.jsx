...
const StatsSection = ({ 
  ...
  const filteredData = useMemo(() => {
    ...
    // Filter by players
    if (selectedPlayers.length > 0 && showPlayerFilter) {
      result = result.filter(row => selectedPlayers.includes(row.PLAYER));
    }
    ...
    return result;
  }, [data, selectedPlayTypes, selectedTeams, selectedPlayers, searchTerm, showPlayTypeFilter, showTeamFilter, showPlayerFilter]);
  ...
  const hasActiveFilters = selectedPlayTypes.length > 0 || selectedTeams.length > 0 || selectedPlayers.length > 0 || searchTerm;
        ...
        <div style={{ borderTop: `1px solid ${colorScheme.cardBorder}` }}>
          ...
          <div style={{ padding: '16px 24px', backgroundColor: '#fafafa', borderBottom: `1px solid ${colorScheme.cardBorder}` }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px' }}>
              ...
              {(showPlayTypeFilter || showTeamFilter || showPlayerFilter) && (
                <button
                ...
                >
                  ...
                  {hasActiveFilters && (
                    <span style={{
                      ...
                      {selectedPlayTypes.length + selectedTeams.length + selectedPlayers.length}
                    </span>
                  )}
                </button>
              )}
            ...
            {showFilters && (
              <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {/* Player Filter */}
                {showPlayerFilter && availablePlayers.length > 0 && (
                  <div>
                    <div style={{ fontSize: '0.7rem', fontWeight: '600', color: colorScheme.secondaryText, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                      Players ({selectedPlayers.length > 0 ? `${selectedPlayers.length} selected` : 'select to filter'})
                    </div>
                      ...
                      {availablePlayers.sort().map(player => (
                        <button
                          ...
                          style={{
                            ...
                            backgroundColor: selectedPlayers.includes(player) ? colorScheme.accent : '#e8e8e8',
                            color: selectedPlayers.includes(player) ? 'white' : colorScheme.primaryText,
                            fontFamily: "'JetBrains Mono', monospace",
                          }}
                        >
                          {player}
...
const PlayerLeaderboardsPage = () => {
  ...
  // Data states
  const [playerOffensiveEfficiency, setPlayerOffensiveEfficiency] = useState([]);
  const [playerShotLocationEfficiency, setPlayerShotLocationEfficiency] = useState([]);
  const [playerShotLocationFrequency, setPlayerShotLocationFrequency] = useState([]);
  const [playerPlaytypeShotFrequency, setPlayerPlaytypeShotFrequency] = useState([]);
    ...
    const loadData = async () => {
      ...
      try {
        ...
        for (const gender of genders) {
          const [effRes, shotEffRes, shotFreqRes, playtypeFreqRes, playTypesRes, teamsRes] = await Promise.all([
            fetch(`${STATIC_DATA_PATH}/${gender}/player-offensive-efficiency.json`),
            fetch(`${STATIC_DATA_PATH}/${gender}/player-shot-location-efficiency.json`),
            fetch(`${STATIC_DATA_PATH}/${gender}/player-shot-location-frequency.json`),
            fetch(`${STATIC_DATA_PATH}/${gender}/player-playtype-shot-frequency.json`),
            fetch(`${STATIC_DATA_PATH}/${gender}/play-types.json`),
            fetch(`${STATIC_DATA_PATH}/${gender}/teams.json`)
          ]);

          if (!effRes.ok || !shotEffRes.ok || !shotFreqRes.ok || !playtypeFreqRes.ok) {
            throw new Error("Data not found. Run: python backend/fetch_and_cache.py");
          }

          const [effData, shotEffData, shotFreqData, playtypeFreqData, playTypesData, teamsData] = await Promise.all([
            effRes.json(),
            shotEffRes.json(),
            shotFreqRes.json(),
          ...
          dataCache[gender] = {
            playerOffensiveEfficiency: effData,
            playerShotLocationEfficiency: shotEffData,
            playerShotLocationFrequency: shotFreqData,
            playerPlaytypeShotFrequency: playtypeFreqData,
            playTypes: playTypesData.play_types || [],
            teams: teamsData.teams || [],
            players: extractPlayers([effData, shotEffData, shotFreqData, playtypeFreqData])
          };
        }
        ...
        setPlayerOffensiveEfficiency(mensData.playerOffensiveEfficiency.data || []);
        setPlayerShotLocationEfficiency(mensData.playerShotLocationEfficiency.data || []);
        setPlayerShotLocationFrequency(mensData.playerShotLocationFrequency.data || []);
        setPlayerPlaytypeShotFrequency(mensData.playerPlaytypeShotFrequency.data || []);
        setAvailablePlayTypes(mensData.playTypes);
        setAvailableTeams(mensData.teams);
        setAvailablePlayers(mensData.players);
        setLastUpdated(mensData.playerOffensiveEfficiency.fetched_at);
  ...
  useEffect(() => {
    if (allData[selectedGender]) {
      ...
      setPlayerOffensiveEfficiency(genderData.playerOffensiveEfficiency.data || []);
      setPlayerShotLocationEfficiency(genderData.playerShotLocationEfficiency.data || []);
      setPlayerShotLocationFrequency(genderData.playerShotLocationFrequency.data || []);
      setPlayerPlaytypeShotFrequency(genderData.playerPlaytypeShotFrequency.data || []);
      setAvailablePlayTypes(genderData.playTypes);
      setAvailableTeams(genderData.teams);
      setAvailablePlayers(genderData.players);
      setLastUpdated(genderData.playerOffensiveEfficiency.fetched_at);
    }
  ...
  // Column definitions
  const playerOffensiveEfficiencyColumns = [
    { key: 'PLAYER', label: 'Player', bold: true, sortable: true },
  ...
  const playerShotLocationEfficiencyColumns = [
    { key: 'PLAYER', label: 'Player', bold: true, sortable: true },
    { key: 'TEAM', label: 'Team', sortable: true },
    { key: 'TOTAL_SHOTS', label: 'Total', sortable: true },
    { key: 'LAYUP_MADE', label: 'Layup M', sortable: true },
    { key: 'LAYUP_PCT', label: 'Layup %', highlight: true, sortable: true },
    { key: 'CLOSE_MADE', label: 'Close M', sortable: true },
    { key: 'CLOSE_PCT', label: 'Close %', highlight: true, sortable: true },
    { key: 'MID_MADE', label: 'Mid M', sortable: true },
    { key: 'MID_PCT', label: 'Mid %', highlight: true, sortable: true },
    { key: 'THREE_MADE', label: '3P M', sortable: true },
    { key: 'THREE_PCT', label: '3P %', highlight: true, sortable: true },
  ];

  const playerShotLocationFrequencyColumns = [
    { key: 'PLAYER', label: 'Player', bold: true, sortable: true },
    { key: 'TEAM', label: 'Team', sortable: true },
    { key: 'TOTAL_SHOTS', label: 'Total', sortable: true },
  ...
  const playerPlaytypeShotFrequencyColumns = [
    { key: 'PLAYER', label: 'Player', bold: true, sortable: true },
    { key: 'TEAM', label: 'Team', sortable: true },
    { key: 'PLAY_TYPE', label: 'Play Type', bold: true, sortable: true },
    { key: 'TOTAL_SHOTS', label: 'Total', sortable: true },
      ...
      <div style={styles.contentWrapper}>
        ...
        <div style={styles.header}>
          <h1 style={styles.title}>
            <User size={28} color={colorScheme.accent} />
            Player Statistics
          </h1>
          <p style={styles.subtitle}>Individual player offensive analytics across the Liberty League</p>
        </div>
        ...
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px', gap: '12px', color: colorScheme.secondaryText }}>
            ...
            <span style={{ fontSize: '1.1rem' }}>Loading player statistics...</span>
          </div>
        ) : error ? (
          ...
          <>
            <StatsSection
              title="Player Offensive Efficiency by Play Type"
              description="Individual player efficiency broken down by play type - shows PPP, shooting percentages, and percentile rankings"
              data={playerOffensiveEfficiency}
              columns={playerOffensiveEfficiencyColumns}
              loading={false}
              error={null}
            ...
            <StatsSection
              title="Player Shot Location Efficiency"
              description="Shooting efficiency by court zone - layups, close range, mid-range, and three-pointers"
              data={playerShotLocationEfficiency}
              columns={playerShotLocationEfficiencyColumns}
              loading={false}
              error={null}
              defaultExpanded={false}
              colorScheme={colorScheme}
            ...
            <StatsSection
              title="Player Shot Distribution"
              description="Shot distribution frequency by court zone - what percentage of shots come from each area?"
              data={playerShotLocationFrequency}
              columns={playerShotLocationFrequencyColumns}
              loading={false}
              error={null}
              defaultExpanded={false}
              colorScheme={colorScheme}
            ...
            <StatsSection
              title="Player Shot Distribution by Play Type"
              description="Shot distribution frequency broken down by play type - where do players shoot from on each offensive action?"
              data={playerPlaytypeShotFrequency}
              columns={playerPlaytypeShotFrequencyColumns}
              loading={false}
              error={null}
              defaultExpanded={false}
              colorScheme={colorScheme}
